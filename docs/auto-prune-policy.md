...
uses `tombstones.coalesceAtPercent`, and also `tombstones.coalesceMinChunks` for
sessions that accumulate many pruned tombstones before extreme pressure, to
replace many old pruned tool-result messages with a single manifest containing
chunk IDs and a `restore_chunks` hint. `tombstones.maxCoalescedEntries` bounds
the number of IDs listed in one manifest. Coalescing is a provider-context
rewrite only; it does not delete or rewrite saved transcript messages.
